 ...
 *                                          *
 *     Sing: non optimal parallelization,   *
 *     memory issues due to storing the     *
 *     whole psi into rank 0                *
 *                                          *
 *     Last mod: 01/20/2018                 *
...
/* Constants and variables for morphologies (Nx = Ny = Nz) */
	...
	const double aE = 405; // 270 (FC) // 80 // 312 // 432 // 248
	const double bE = 405; // 270 (FC) // 86 // 376 // 520 // 248
...
/* Phase Field parameters */
	...
	const double ep    = -0.75; // -0.7 CHANGED !!!!!!!!!!!!!!
	const double q0    =  1.0;
...
/* B. Initial condition - Read profile data */
	...
	/** Open file and obtain IC for global psi **/
    	...
    	std::ifstream psidata("/home/evitral/cosMorph/morphSurf/psiHole.dat");
...
/* Save psi and surface info (under count condition) */
		...
		// Shouldn't need to comput the whole thing, but this avoid some mess				

			MPI::COMM_WORLD.Gather(psi_local.data(),alloc_local,

 ...
 *     Bump: algorithm for tracking bump    *
...
/* Ints and doubles for surface info */
	...
	int index1, index2, index3, index4, track, k2;
...
/* Load/save parameters */
	...
	std::string strBox = "box4/saveBump/";
...
/* Constants and variables for morphologies (Nx = Ny = Nz) */
	...
	const double aE = 550; // 270 (FC) // 80 // 312 // 432 // 248 // 810
	const double bE = 550; // 270 (FC) // 86 // 376 // 520 // 248 // 810
...
/* Save psi and surface info (under count condition) */
	...
	/** Appropriate way to compute curvatures for a TFCD **/
		...
		for ( k = Nz-1; k > -1; k-- ) 
		{
			...
			if ( std::abs(psi_local[index]) > 0.6 & track == 0 ) // 0.7 ; results are better when looking for this 0
			{
				track = 1;
				...
				//while (std::abs(psi_local[index3]+psi_local[index4]) > std::abs(psi_local[index3]))
				while(std::abs(psi_local[index3]) > 0.1)
				{
				    k2++;
				    index3++;
				    index4++;
			  ...
			  //{
				...
				if( std::abs(psi_local[index3]) > std::abs(psi_local[index4]) ) // >
				{
					index = index4;
					k2 ++;
				}
